    ...
    for feature in features:
        ...
        if hasattr(feature, '__geo_interface__'):
            ...
            if feature['type'] == 'Feature':
                ...
                # ... or a geometry directly
                for pt in _geom_points(feature):
                    yield pt

        elif isinstance(feature, ['list', 'tuple']):
            # Here only points with lat,lon are allowed
            if len(feature) == 2:
                yield tuple(feature)
            else:
                raise InvalidFeatureError(
                    "Unknown object: Not a GeoJSON Point feature, "
                    "an object with __geo_interface__, or a tuple of size 2"
                    ":\n{0}".format(feature))
